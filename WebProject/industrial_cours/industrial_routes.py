...
def get_db_connection():  # связь с базой данных
...
@industrial_bp.route('/')
def industrial_course():  # получения данных из базы данных(тем и заданий)
...
def get_tasks(topic_id):  # получение заданий
    conn = get_db_connection()
    try:
        tasks = conn.execute('''
...
def submit_solution(task_id):  # добавление своего решения
...
def get_solutions(task_id):  # получение своих решений
    conn = get_db_connection()
    try:
...
def pygame1_1_task():  # страница задания крест из pygame1
